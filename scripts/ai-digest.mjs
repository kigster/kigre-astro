...
async function fetchArxiv() {
  ...
  for (const cat of CATEGORIES) {
    ...
    for (const e of entries) {
      const grab = (tag) => (e.match(new RegExp(`<${tag}>([\\s\\S]*?)</${tag}>`)) || [, ''])[1].trim();
...
async function verify(body, papers) {
  ...
  try {
    const json = JSON.parse(out.replace(/```json|```/g, '').trim());
    console.log(`Stage 4 VERIFY: ${json.ok ? 'PASS' : 'ISSUES'} (${json.issues?.length || 0})`);
...
function emit(body, verifyResult, paperCount) {
  ...
  console.log(`Stage 5 EMIT: ${outPath} (draft, ${flagged ? 'FLAGGED' : 'clean'})`);
  return outPath;
}
