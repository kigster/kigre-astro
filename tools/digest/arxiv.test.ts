...
import { buildArxivUrl, parseArxivFeed, type Paper } from "./arxiv";
...
describe("buildArxivUrl", () => {
  test("bare category query without a theme", () => {
    expect(buildArxivUrl("cs.AI", 40)).toBe(
      "http://export.arxiv.org/api/query?search_query=cat%3Acs.AI" +
        "&sortBy=submittedDate&sortOrder=descending&max_results=40",
    );
  });

  test('ANDs comma-separated theme phrases as OR-ed all:"…" clauses', () => {
    const url = buildArxivUrl("cs.LG", 10, "agents, tool use");
    expect(decodeURIComponent(url)).toContain(
      'search_query=cat:cs.LG AND (all:"agents" OR all:"tool use")',
    );
  });
});
