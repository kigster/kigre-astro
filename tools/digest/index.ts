 ...
 *
 * NOTE: the rich CLI (commander flags: --provider, --days-back, --theme,
 * --article-depth, --article-type, --compile-sources) is .plans/004 — this file
 * preserves the original single-mode digest behaviour, now Genkit-driven.
 */
...
const DAYS_BACK = Number(process.env.DIGEST_DAYS_BACK ?? 7)
const MAX_PAPERS = 40
...
async function main(): Promise<void> {
  ...
  console.log(`FETCH: ${papers.length} papers in the last ${DAYS_BACK} days`)
  if (papers.length === 0) {
  ...
  // ---------- 4. VERIFY ----------
  const abstracts = papers.map((p) => `"${p.title}": ${p.summary}`).join('\n\n')
  ...
  console.log(`VERIFY: ${verdict.ok ? 'PASS' : 'ISSUES'} (${verdict.issues.length})`)
