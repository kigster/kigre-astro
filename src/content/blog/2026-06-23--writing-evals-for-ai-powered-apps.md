...
from end to end. No ML PhD required. If you can write an [RSpec test](/2020/09/07/writing-cli-tools-ruby-migrating-github-issues-to-pivotal-tracker.html), you can write an
eval.
...
1. A **dataset** of inputs paired with what we expect (often called the _ground truth_, or _golden set_).
...
> [!IMPORTANT]
>
> If there's a single thread tying this all together, it's this: **the profession is growing up and learning to measure the measurer.**
>
> When you thinka about it, it almost has parallels to the recent [discoveries by Google about Quantum Computing](https://techcrunch.com/2024/12/10/google-says-its-new-quantum-chip-indicates-that-multiple-universes-exist/). That's the same idea this post was all about, just one level higher. **Don't trust the model — measure it**. **Don't trust the judge — calibrate it, and don't assume a committee of judges fixes it**. Don't trust the leaderboard — build your own. Don't even fully trust your own error bars — widen them. It's measurement all the way down, and after twenty-five years of green dots, I find that turtles-all-the-way-down rigor weirdly reassuring.
>
> <br />
> Now go write ten cases for whatever AI feature is making you nervous. I'll wait. 😂

## Latest Papers on the Subject

Having looked through some of the papers over the last few weeks on the subject, 
I am finding a couple of themes really come through.

### 1. The Judge needs a Judge

Remember my hand-wavy "validate the judge against human labels, it's turtles for a little while" aside up in the LLM-as-judge
...
exact bias I warned you about: a judge being too soft on its own work. 

Back in April, [Self-Preference Bias in Rubric-Based Evaluation](https://arxiv.org/abs/2604.06996)
...
"Fine," you think, "I'll convene a _panel_ of judges and let the bias wash out."
...
judges that are flawlessly _consistent_ and consistently _biased_ at the same time. 

The one that should make anyone who ships sit up (or sit down, I dunno) is — [Catching One in Five](https://arxiv.org/abs/2606.10315)
...
watch it proven about our graders. 

> [!TIP]
> 
> The lesson is unglamorous and non-negotiable: if you lean on an LLM judge, you owe it a calibration set of human labels, you owe it _more_ scrutiny the squishier the task, and you don't get to buy your way out by throwing more judges at it. The turtles are real. Budget for them.

### 2. Somebody Else's Leaderboard is Not Your Eval.

This is the academic version of the rant I went on earlier — 
"the benchmarks aren't running _your_ prompt against _your_ data" — and it shows up at every layer of the stack. 

At the benchmark layer, an April meta-analysis, [The LLM Effect on IR Benchmarks](https://arxiv.org/abs/2604.05766)
(Staudinger, Kusa & Hanbury), finds measurable contamination quietly propping up
reported gains. 

At the leaderboard layer, May's [AgentAtlas](https://arxiv.org/abs/2605.20530)
...
how a system behaves once deployed. 

And at the most damning layer,
...
made it sound like a confidence interval would have your back. 

Well, April's [Hidden Measurement Error in LLM Pipelines](https://arxiv.org/abs/2604.11581) (Messing)
...
know that the easy ones lie to you in the optimistic direction. 

**Make the set bigger than feels necessary, and then a little bigger than that.**

> [!TIP]
>
> If there's a single thread tying the season together, it's this: **the profession is 
> growing up and learning to measure the measurer.** 
> 
> That's the same move this whole post has been about, just one level higher. 
> 
> Don't trust the model — measure it. Don't trust the judge — calibrate it, and don't 
> assume a committee of judges fixes it. 
>
> Don't trust the leaderboard — build your own. Don't even fully trust your own error bars — widen
...
Now go write fifty evals for whatever AI feature is making you nervous. I'll wait.
...
> [!CAUTION]
> 
> **Conclusion**: As I wrote this article about Evals, I went on quite a few rants and claimed a few ideas that, despite being born instinctually, happen to be very much in line with the current scientific literature. I read (scanned, summarized it in some cases) so you don't have to.
...
- ... and this outdated, a bit tedious, but still marginally useful article written by the folks at sOpenAI about [Eval Driven System Design - From Prototype to Production](https://developers.openai.com/cookbook/examples/partners/eval_driven_system_design/receipt_inspection) — might be worth a read when you've run out of everything else to watch, read and learn.
